...
terrenos de montaña donde confluyen precipitaciones intensas
y laderas de fuerte pendiente en suelos tropicales \citep{aristizabal2026landslide,
...
y el 80\,\% ocurrieron en los flancos de las cordilleras de los Andes
\citep{aristizabal2020landslide}. Este patrón espacial refleja la
combinación de laderas de fuerte pendiente, perfiles de meteorización profundos, y presencia de eventos de lluvias intensos \citep{aristizabal2026landslide}. 
...
y las covariables en escala logit. En el otro extremo, los métodos de aprendizaje automático (\textit{machine learning} - bosques aleatorios, redes neuronales,
...
que equilibra ambas ventajas: permiten relaciones no lineales entre
...
susceptibilidad es un producto derivado del análisis, no corresponde al objetivo del presente estudio, por lo cual no se incorporaron dentro del análisis otras variables importantes en la ocurrencia de movimientos en masa como geología y coberturas del suelo.
...
Metropolitana, entre ellos Medellín, con una población de aproximadamente
...
La susceptibilidad por movimientos en masa depende, entre otras, de variables morfométricas
...
lineales. Modelar incorrectamente estas no
linealidades sesga las predicciones espaciales de susceptibilidad y
llevar a errar en la identificación de zonas críticas.

Los Modelos Lineales Generalizados (GLM, \textit{Generalized Linear
Models}) \citep{nelder1972generalized, mccullagh1989generalized}
constituyen una extensión del modelo lineal ordinario por mínimos
cuadrados que unifica, bajo un marco teórico común, una amplia familia
de distribuciones para la variable respuesta. Su formulación se
articula en tres componentes articulados.

\begin{enumerate}
    


\item  \textbf{Componente aleatoria}: se asume que $Y_i$ pertenece a la
familia exponencial de distribuciones, con función de densidad o masa
de probabilidad de la forma
\begin{equation}
    f(y_i;\,\theta_i,\phi) = \exp\!\left[
        \frac{y_i\,\theta_i - b(\theta_i)}{a(\phi)} + c(y_i,\phi)
    \right],
    \label{eq:glm}
\end{equation}
donde $\theta_i$ es el parámetro canónico natural, $\phi > 0$ el
parámetro de dispersión, y $b(\cdot)$, $a(\cdot)$, $c(\cdot)$ son
funciones que caracterizan cada distribución particular. De esta
parametrización se derivan directamente
$\mathrm{E}[Y_i] = \mu_i = b'(\theta_i)$ y
$\mathrm{Var}[Y_i] = b''(\theta_i)\,a(\phi)$, lo que desacopla la
media condicional de la varianza y permite heterocedasticidad
estructuralmente coherente con la distribución asumida.
\item \textbf{Predictor lineal}: $\eta_i = \mathbf{x}_i^\top
\boldsymbol{\beta}$, donde $\mathbf{x}_i \in \mathbb{R}^p$ es el
vector de covariables observadas y $\boldsymbol{\beta} \in \mathbb{R}^p$
el vector de parámetros a estimar.
\item \textbf{Función de enlace} $g(\cdot)$: función monótona y
diferenciable que relaciona la media condicional con el predictor
lineal mediante $g(\mu_i) = \eta_i$, garantizando compatibilidad entre
el dominio irrestricto de $\eta_i \in \mathbb{R}$ y el dominio natural
de $\mu_i$.
\end{enumerate}

 Los parámetros $\boldsymbol{\beta}$ se estiman por máxima
verosimilitud mediante el algoritmo de \textit{mínimos cuadrados
reponderados iterativamente} (IRLS, \textit{Iteratively Reweighted
Least Squares}), que converge de forma garantizada para cualquier
distribución de la familia exponencial. La regresión logística, caso
particular con $Y_i \sim \mathrm{Bernoulli}(\mu_i)$ y enlace logit
$g(\mu_i) = \ln[\mu_i/(1-\mu_i)]$, es el GLM empleado
convencionalmente para modelar la susceptibilidad por movimientos en
masa. La restricción estructural de todos los GLM reside en que el
predictor lineal $\eta_i$ es una función \emph{lineal y paramétrica}
de las covariables: aunque $g(\cdot)$ permite modelar respuestas no
normales, la contribución de cada predictor a $\eta_i$ permanece fija
y sin posibilidad de capturar curvaturas, umbrales o inversiones de
efecto en la relación predictor--respuesta.
...
Las funciones de base actúan como bloques de distintas
...
Sin ninguna restricción, la curva $s_j$ podría ajustarse exactamente a
cada punto de entrenamiento mediante oscilaciones erráticas. Este
fenómeno, denominado sobreajuste, produce curvas que
reproducen bien los datos de entrenamiento pero predicen mal en zonas
sin cobertura de inventario. Para evitarlo, el GAM incorpora una
penalización de rugosidad: una restricción matemática que
desincentiva las curvas muy oscilantes en favor de formas suaves y
geomorfológicamente interpretables. La rugosidad de cada curva $s_j$
se cuantifica mediante la integral del cuadrado de su segunda derivada:
...
en cada punto del predictor: si $s_j''(x) = 0$ en todo su dominio, la
curva es una recta; cuanto mayor sea $|s_j''(x)|$, más abruptamente
cambia de dirección la curva en ese punto. La integral $J(s_j)$
acumula esa curvatura a lo largo de todo el rango del predictor: vale
cero para una recta perfecta y crece cuanto más ondulada sea la
función. Intuitivamente, $J(s_j)$ es una medida de la complejidad de la curva: una función que oscila repetidamente entre valores altos
y bajos tendrá un $J$ mucho mayor que otra con un patrón suave de una
sola cima, como la relación esperada entre una covariable y probabilidad de
deslizamiento.

El modelo se ajusta maximizando la log-verosimilitud
penalizada, es decir, buscando simultáneamente las curvas que mejor
explican la distribución espacial de los deslizamientos \emph{y} que
...
donde $\ell(\boldsymbol{\gamma})$ cuantifica la bondad del ajuste
(cuán bien las curvas estimadas reproducen la distribución observada
de deslizamientos y no-ocurrencias en el territorio) y
$\lambda_j > 0$ es el parámetro de suavizado de cada término.
El signo negativo de la penalización implica que curvas muy rugosas
reducen el valor de $\ell_P$, penalizando el sobreajuste. El
parámetro $\lambda_j$ actúa como un regulador de rigidez: un
valor pequeño (cercano a cero) deja libre a la curva para adoptar
cualquier forma que mejore el ajuste, con riesgo de sobreajuste; un
valor muy grande fuerza a la curva a ser casi recta, de modo que la
función de suavizado degenera en un coeficiente lineal y el GAM se
reduce a la regresión logística clásica. 

Los GAM engloban, por tanto,
a los GLM como un caso límite particular: la regresión logística es
el GAM en el que se impone máxima rigidez ($\lambda_j \to \infty$) a
todos sus términos, convirtiendo cada curva $s_j$ en una simple
pendiente fija.
...
subajuste y eliminan las relaciones no lineales de interés.
...
En el Valle de
Aburrá, los sectores bajos y medios de ladera presentan
la mayor densidad
constructiva sobre ladera (Figura~\ref{fig:hist}).

\item \textbf{Pendiente} ($\beta$, grados): Está directamente relacionada con la estabilidad de laderas, dado que
la componente tangencial de la gravedad, que genera tensión de corte
en el plano de falla, aumenta con $\sin\beta$.
...
de las seis variables morfométricas utilizadas.
...
La Figura~\ref{fig:mapa_reg} presenta el mapa de susceptibilidad, con paleta de colores de verde
...
resultaron altamente significativos ($p < 0{,}001$). El Q-Q de residuos de devianza muestra un
...
urbanización de ladera en esa franja altitudinal.

\item $s(\text{pendiente})$: presenta un incremento sostenido y pronunciado hasta
los 35°--40°, seguido de una meseta o leve descenso.
...
efecto más pronunciado de todos los predictores, sugiriendo que la
...
excelente. La Tabla~\ref{tab:metrics} resume todas las
...
al omitir cada predictor del modelo completo. El TWI es
claramente el predictor dominante, con una contribución varias veces
superior a la de las demás variables. La pendiente ocupa el
segundo lugar, seguida de curvatura de perfil y
curvatura planiforme. La elevación y el
aspecto tienen contribuciones menores pero estadísticamente
significativas.
...
Los resultados confirman que los GAM constituyen una herramienta metodológicamente robusta para cuantificar relaciones no lineales entre variables morfométricas e inestabilidad de laderas. 
...
por bloques espaciales, separando los grupos de entrenamiento por microcuencas o cuadrantes
disjuntos, puede permitir obtener estimaciones más conservadoras de la capacidad predictiva real del modelo para transferir dichos resultados a otras zonas. Sin embargo ese no es el alcance de este trabajo. Desde el punto de vista del autor esta transferencia no es adecuada, ya que cada territorio presenta características particulares que no permite generalizar la relación causa-efecto para otras zonas. Es recomendable identificar estas variables y su efecto en la ocurrencia de movimientos en masa para cada territorio.
...
efecto de agotamiento (\textit{exhaustion effect}), ha sido
...
corresponden a afloramientos en roca con escaso perfil de meteorización, la coherencia geomecánica
...
los escarpes son intrínsecamente estables a este tipo de movimientos en masa (deslizamientos y flujos) por que han
liberado ya todo su material disponible. Un modelo de regresión logística
que asumiese linealidad en el logit sobre-estimaría la susceptibilidad en
...
sesgo de detección altitudinal. En elevaciones
...
la respuesta de los deslizamientos a la precipitación. Desde una perspectiva física, la dominancia del TWI
...
y la zona de mayor densidad constructiva. 

%===================================================================
\section{Conclusiones}
%===================================================================

Este estudio demuestra que los Modelos Aditivos Generalizados con
regresión logística y las funciones \textit{thin plate splines} constituyen un marco
estadístico adecuado para cuantificar el efecto no lineal de variables
morfométricas sobre la ocurrencia de movimientos en masa en el Valle de
Aburrá. El modelo alcanzó AUC~=~0{,}908 y TSS~=~0{,}816 en un conjunto
de prueba independiente, desempeño que lo sitúa en la categoría
\textit{excelente} y que supera el rango central reportado en la
literatura para modelos GAM de susceptibilidad en contextos de montaña
comparables. Esta capacidad predictiva se logra sin sacrificar la
interpretabilidad: las funciones de suavizado permiten visualizar el
efecto individual de cada predictor en la escala de la probabilidad de
falla, una ventaja crítica frente a los métodos de \textit{machine
learning} en entornos donde los modelos deben ser auditables y
justificables ante autoridades.

El análisis de las funciones de suavizado revela que el control
hidrológico, representado por el TWI, es el factor condicionante
dominante, seguido de la pendiente y las curvaturas
topográficas. Este resultado es físicamente coherente: la acumulación
de presión de poros en zonas de convergencia topográfica es el
mecanismo desencadenante principal de los movimientos en masa
superficiales en los suelos saprolíticos del valle bajo precipitación
intensa. Las funciones estimadas identifican además rangos morfométricos
críticos con valor operativo directo: pendientes entre 20° y 45°,
elevaciones entre 1\,500 y 2\,000\,m, orientaciones oeste-noroeste,
zonas cóncavas en planta y en perfil, y TWI superior a 5. Estos rangos
caracterizan las condiciones morfométricas de mayor susceptibilidad y
constituyen umbrales de referencia para la zonificación territorial.
